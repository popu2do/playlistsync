...
var (
	cookieSecretRegex = regexp.MustCompile(`(?i)(sp_dc|sapisid|__secure-[a-z0-9_-]+|sid|hsid|ssid|apisid|nid|login_info|account_chooser|auth_token|access_token|refresh_token|csrf_token)=([^\s;"']+)`)
	authHeaderRegex   = regexp.MustCompile(`(?i)(sapisidhash|bearer|basic)\s+([^\s;"']+)`)
